...
class TestInitializeContracts(unittest.TestCase):
    ...
    def test_capability_binding_validate_foundation(self):
        # Reset states
        project_contract.initialized = False
        execution_contract.initialized = False
    ...
    def test_home_route(self):
        ...
        self.assertTrue(data["foundation_ready"])

    def test_validate_foundation_route(self):
        response = self.app.get('/validate-foundation')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertIn("repository_ready", data)
        self.assertIn("foundation_ready", data)
        self.assertIn("details", data)
        self.assertTrue(data["repository_ready"])
        self.assertTrue(data["foundation_ready"])
        self.assertIn("project_contract", data["details"])
        self.assertIn("execution_contract", data["details"])
        self.assertTrue(data["details"]["foundation_ready"])
